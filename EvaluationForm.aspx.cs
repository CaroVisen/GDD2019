...
public partial class EvaluationForm : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        jefeGroup.Value = groupId.ToString();

        SetButtonSend();

        SetButtons(profile);

        if (pId.Value.Equals("1"))
            tabSE.Visible = true;
        else
            tabSE.Visible = false;
    }
    private void SetButtons(int profile)
    {
        ...
        if (profile == 1)
        {
            ...
            btnSave.Visible = _visible || ("10,12".IndexOf(_params[2].ToString(), 0) >= 0);
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        ...
        //catch (Exception ex)
        //{
        //    Page.RegisterStartupScript("startupScript", "<script language=JavaScript type=\"text/javascript\">alert(\"Error:" + ex.Message + "\");\");</script>");
        //}
    }
    ...
    private void SavePM()
    {
        ...
        for (int i = 0; !exit; i++)
        {
            ...
            if (!exit)
            {
                if (answer.GetType().ToString() == "System.Web.UI.WebControls.TextBox" || answer.GetType().ToString() == "System.Web.UI.WebControls.DropDownList")
                {
                    ...
                    if (answer.GetType().ToString() == "System.Web.UI.WebControls.TextBox")
                    {
                        ...
                        if (!myControl.ReadOnly)
                        {
                            ...
                            _field++;
                            skinId = myControl.SkinID.ToString();
                        }
                        else
                            readyOnly = true;
                        myControl.Dispose();
                    ...
                    if (answer.GetType().ToString() == "System.Web.UI.WebControls.DropDownList")
                    {
                        ...
                        {
                            values[_field] = myControl.SelectedValue;
                            _field++;
                            skinId = myControl.SkinID.ToString();
                        }
                        else
                    ...
                    if (_field == 5)
                    {
                        ...
                        SQLHelper.ExecuteNonQuery(Cache["ApplicationDatabase"].ToString(), "[SaveResultImprovementPlan]", new object[] { Convert.ToInt16(skinId), 3, values[0], values[2], values[1], values[3], null, values[4] });
    ...
    private void SaveSeg()
    {
        ...
        for (int i = 0; !exit; i++)
        {
            ...
            if (!exit)
            {
                ...
                {
                    TextBox myControl = (TextBox)answer;
                    if (!myControl.ReadOnly && myControl.Attributes["class"] == "Seg")
                    {
                        if (myControl.Text.Length > 5)
                            SQLHelper.ExecuteNonQuery(Cache["ApplicationDatabase"].ToString(), "AddFollowup", new object[] { Convert.ToInt32(myControl.SkinID), myControl.Text });
                    }
                    myControl.Dispose();
                }
                answer.Dispose();
        ...
        for (int i = 0; !exit; i++)
        {
            ...
            if (!exit)
            {
                if (answer.GetType().ToString() == "System.Web.UI.WebControls.TextBox")
                {
                    ...
                    description = myControl.Text.ToString();
                    assessment = values[1];
                    SQLHelper.ExecuteNonQuery(Cache["ApplicationDatabase"].ToString(), "SaveResultVariable", new object[] { values[1], description, _delete });
                    description = string.Empty;
